...
```latex
% Article workflow
\usepackage[article, layout, tikz, extramath, metadata]{csextras}

% Book workflow
\usepackage[book, layout, index, lettrine, epigraph]{csextras}
...
| Option | Effect |
|--------|--------|
| `article` | Loads `csamsmath` |
| `book` | Loads `csbook` |

`article` and `book` are mutually exclusive. If neither is given, `csextras` infers the mode from an already-loaded `csamsmath` or `csbook`.
If neither base package is requested or already loaded, `csextras` loads `babel` without language options so that documents keep control over language configuration.
...
| `hyperrefcolors` | Sets `citecolor`, `linkcolor`, `urlcolor` in `hyperref` at `\begin{document}` |
