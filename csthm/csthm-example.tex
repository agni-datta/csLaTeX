...
\usepackage[colorlinks=true,linkcolor=SteelBlue4,citecolor=SteelBlue4,urlcolor=SteelBlue4]{hyperref}
\usepackage{xurl}
...
\begin{abstract}
	The \textsf{csthm} package provides theorem-like environments for computer science and related technical disciplines. It offers three visual styles, modern (\texttt{fancy}), standard (\texttt{normal}), and traditional (\texttt{oldschool}), each with numbered and unnumbered forms. The package declares its theorem layer directly with \textsf{ntheorem} and integrates it with \textsf{mdframed} and \textsf{cleveref} for cross-references, configurable headers, and consistent presentation. The traditional style is the most conservative choice, while the fancy and normal styles provide progressively stronger visual structure.
\end{abstract}
...
\textsf{ntheorem} foundation, the package configures and extends its
...
\begin{itemize}[label={--}]
	\item \textsf{amsmath}, \textsf{amssymb}, and \textsf{ntheorem}: These packages provide mathematical typesetting, symbols, AMS-compatible proof commands, and the theorem infrastructure used by \textsf{csthm}.
...
the lower-level \textsf{ntheorem} package directly, which offers finer-grained control and a broader API
...
The \textsf{csthm} package uses \textsf{ntheorem} directly to implement all theorem-like environments. This approach ensures that
...
\begin{itemize}[label={--}]
	\item All theorem-like environments use native \textsf{ntheorem} declarations for uniform behavior.
